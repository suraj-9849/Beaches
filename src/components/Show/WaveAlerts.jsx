...
const extractWaveData = (message) => {
  const highWaveMatch = message.match(
    /High waves?\s*in\s*the\s*range\s*of\s*([\d.]+)\s*-\s*([\d.]+)\s*m/i
  );
  const highWave = highWaveMatch
    ? ((parseFloat(highWaveMatch[1]) + parseFloat(highWaveMatch[2])) / 2).toFixed(1)
    : 'N/A';

  const swellSurgeMatch = message.match(
    /Swell waves? in the range of \d+\.?\d* - \d+\.?\d* sec period with (\d+\.?\d*) - (\d+\.?\d*) m height/i
  );
...
const WaveAlerts = ({ district, onWaveDataUpdate }) => {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        ...
        const highWaveAlerts = JSON.parse(data.HWAJson).filter(
          (alert) => alert.District.toLowerCase() === district.toLowerCase()
        );
        setHighWaveAlerts(highWaveAlerts);

        const swellSurgeAlerts = JSON.parse(data.SSAJson).filter(
          (alert) => alert.District.toLowerCase() === district.toLowerCase()
        );
  ...
  const handleWaveDataUpdate = useCallback(
    (alerts) => {
                  ...
                  <div key={alert.OBJECTID} className="bg-white p-4 rounded-lg shadow-md">
                    <div className="flex items-center mb-4">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-sm">
                          Issue Date:{' '}
                          {dayjs(alert['Issue Date'], 'DD-MM-YYYY').format('MMM DD, YYYY')}
                        </p>
                      </div>
                    </div>
