...
const CarouselWithDetails = ({ name, beachLocation, beachId }) => {
  ...
  const handlePhotoUpload = async () => {
    ...
    try {
      ...
      const sanitizedFileName = newImage.name.replace(/\s+/g, '-').replace(/[^a-zA-Z0-9.-]/g, '');
      ...
      const {
        data: { publicUrl },
        error: urlError
      } = supabase.storage.from('beach-images').getPublicUrl(`photos/${encodedFileName}`);
  ...
  return (
    <div className="w-full mx-auto p-4 md:p-8 rounded-3xl shadow-2xl">
      ...
      >
        ...
        <div className="mb-4">
          <Rating count={5} size={24} value={rating} onChange={handleRating} className="mb-2" />
          <textarea
            value={newReview}
      ...
      >
        ...
        <div className="flex justify-end mt-4">
          <button className="px-4 py-2 bg-blue-500 text-white rounded-lg" onClick={navigateToLogin}>
            Go to Login
          </button>
        </div>
      </Modal>
