...
const Footer = () => {
  return (
    <footer className="fixed bottom-0  flex lg:hidden items-center w-full left-0  h-[8vh] z-50 bg-gradient-to-r from-emerald-400 to-blue-500 shadow-lg">
      <div className="flex justify-around items-center w-full h-full ">
        <Link to="/search">
          <FooterButton icon={<Search size={24} />} label="Search" />
        </Link>
        <Link to="/map">
          <FooterButton icon={<Map size={28} />} label="Map" />
        </Link>
        <Link to="/BookMark">
          <FooterButton icon={<Bookmark size={24} />} label="Bookmark" />
        </Link>
      </div>
    </footer>
  );
